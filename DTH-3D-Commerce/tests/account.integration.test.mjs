...
test('Account API against real MongoDB', async t => {
  ...
  try {
    ...
    await t.test('Checkout persists an actual simulated order and duplicate retry is idempotent', async () => { const body = { items: [{ productId: p.id, vehicleId, quantity: 2 }], idempotencyKey: randomUUID(), demoAcknowledged: true, expectedTotal: p.price * 2 }; const r = await request('/orders', 'POST', body, a); assert.equal(r.status, 201); orderId = r.data.data.id; assert.equal(r.data.data.total, p.price * 2); const repeat = await request('/orders', 'POST', body, a); assert.equal(repeat.data.data.id, orderId); assert.equal(await Order.countDocuments({ userId: a.id }), 1); });
