...
function AuthForm() {
  ...
  const lock = useRef(false), live = useLive(), errorBox = useRef(null);
  ...
  return <div className={s.authLayout}>
    <aside className={s.brandPanel} aria-label="DTH Parts Studio">
      ...
      <div className={s.orbitArt} aria-hidden="true"><span /><span /><span /><b>YOUR<br />NEXT<br /><em>BUILD.</em></b></div>
      <div className={s.brandFoot}><span>01 / YOUR SPACE</span><Icon name="arrow" /></div>
    </aside>
...
function VehicleForm() {
  const store = useStore(), live = useLive(), lock = useRef(false);
  ...
  return <div className={s.vehicleGrid}>
    <section className={s.panel}>
      ...
      <form onSubmit={e => { e.preventDefault(); if (selected) void save(selected.id); }} aria-busy={busy}>
        <label className={s.field}>Make<select required disabled={busy} value={make} onChange={e => { setMake(e.target.value); setModel(''); setYear(''); setMessage(''); }}><option value="">Choose make</option>{unique(vehicles.map(v => v.make)).map(v => <option key={v}>{v}</option>)}</select></label>
        <label className={s.field}>Model<select required disabled={!make || busy} value={model} onChange={e => { setModel(e.target.value); setYear(''); setMessage(''); }}><option value="">Choose model</option>{unique(vehicles.filter(v => v.make === make).map(v => v.model)).map(v => <option key={v}>{v}</option>)}</select></label>
        <label className={s.field}>Year<select required disabled={!model || busy} value={year} onChange={e => { setYear(e.target.value); setMessage(''); }}><option value="">Choose year</option>{unique(vehicles.filter(v => v.make === make && v.model === model).map(v => v.year)).sort((a, b) => b - a).map(v => <option key={v}>{v}</option>)}</select></label>
    ...
    <aside className={`${s.panel} ${s.vehicleSummary}`}>
      ...
      <p className={s.fine}>Compatibility uses the demonstration dataset.</p>
...
function OrderContent({ order }) {
  ...
  return <section className={s.panel}>
    ...
    <ul className={s.receipt}>{order.lines.map(line => {
      ...
      return <li key={`${line.productId}:${line.vehicleId}`}><div><strong>{line.name}</strong><p>{vehicle ? vehicleName(vehicle) : line.vehicleId}</p><small>{line.quantity} × {formatMoney(line.unitPrice)}</small></div><b>{formatMoney(line.lineTotal)}</b></li>;
    })}</ul>
...
function OrderDetail({ id, onBack }) {
  ...
  useEffect(() => {
    ...
    loadAccountOrder(id).then(order => { if (live) setResult(order); }).catch(e => {
      if (!live) return;
  ...
  return <><button type="button" className={s.back} onClick={onBack}>← All orders</button>{error ? <div className={s.panel}><Notice error>{error}</Notice><button className={s.secondary} onClick={() => setRetry(v => v + 1)}>Try again</button></div> : result ? <OrderContent order={result} /> : <Busy label="Loading order…" />}</>;
}
...
export default function AccountPage() {
  const store = useStore();
  useEffect(() => { document.title = 'Account — DTH Parts Studio'; }, []);
  return <section className={`dth-container ${s.root}`}>
    {store.authLoading ? <Busy label="Checking your session…" /> : PREVIEW ? <div className={s.preview}><h1>Your account.</h1><p>Accounts and saved orders are available in API mode.</p><Link className={s.primary} to="/shop">Explore parts<Icon name="arrow" /></Link></div> : store.user ? <MemberArea key={store.user.id} /> : <AuthForm />}
  </section>;
}
