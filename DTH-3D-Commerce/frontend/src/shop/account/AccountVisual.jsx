...
class VisualBoundary extends Component { state={failed:false}; static getDerivedStateFromError(){return {failed:true};} componentDidCatch(){this.props.onFailure();} render(){return this.state.failed?null:this.props.children;} }
...
export default function AccountVisual({ phase: suppliedPhase, compact=false }) {
  ...
  useEffect(()=>{if(!visible||ready||failed)return;const timer=setTimeout(onFailure,motion.sceneTimeoutMs);return ()=>clearTimeout(timer);},[visible,ready,failed,onFailure]);
  const running=visible&&inTab&&!paused&&!signal.reduced;
  ...
  return <div className={s.visual} ref={root} data-compact={compact} data-account-scene={failed?'fallback':ready?'ready':'loading'} data-motion={running?'on':'off'} data-assembly={expanded?'expanded':'assembled'} data-phase={phase}
    ...
    {visible&&!failed&&<div className={s.visualCanvas} data-ready={ready} aria-hidden="true"><VisualBoundary onFailure={onFailure}><Suspense fallback={null}><Scene running={running} phase={phase} expanded={expanded} pointer={pointer} onReady={onReady} onFailure={onFailure}/></Suspense></VisualBoundary></div>}
